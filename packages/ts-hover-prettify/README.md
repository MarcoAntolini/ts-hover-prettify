# ts-hover-prettify

This tool helps you make more readable and prettier hover tooltips for your Typescript code.

## Usage

First, you need to install the package:

```bash
npm add -D ts-hover-prettify
```

And then you have to add a file `prettify.d.ts` to your project with one of the following:

```typescript
import "ts-hover-prettify/global";
```

Or import the type directly:

```typescript
import type { Prettify } from "ts-hover-prettify";
```

That's it! Now you can use `Prettify` inside your project.

## Example

Let's say you have a type like this:

```typescript
type Intersected = {
  a: string;
} & {
  b: number;
} & {
  c: boolean;
};
```

Hovering over `Intersected` would show you the following tooltip:

```typescript
/**
 * { a: string; } & { b: number; } & { c: boolean; }
 */
```

But wrapping it inside of `Prettify` you can make it more readable:

```typescript
type Intersected = Prettify<
  {
    a: string;
  } & {
    b: number;
  } & {
    c: boolean;
  }
>;

/**
 * {
 *   a: string;
 *   b: number;
 *   c: boolean;
 * }
 */
```

## VS Code / Cursor extension

Prefer zero-config? Install the [ts-hover-prettify extension](https://github.com/MarcoAntolini/ts-hover-prettify/tree/master/packages/vscode-extension) instead of this npm package.
